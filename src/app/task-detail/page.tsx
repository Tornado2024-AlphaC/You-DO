  ...
  return (
    <div className="w-full max-w-sm mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center mb-4">
        ...
        <span className="text-green-600 ml-auto">{Math.floor(totalMinutes / 60)}時間 {totalMinutes % 60}分</span>
      </div>
      ...
      <div className="flex gap-4">
        <button onClick={handleCancel} className="px-4 py-2 bg-red-200 text-red-600 rounded-full">キャンセル</button>
        <button onClick={() => alert('保存')} className="px-4 py-2 bg-blue-200 text-blue-600 rounded-full">保存</button>
      </div>
    </div>
  );
};
