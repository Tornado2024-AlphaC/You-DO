...
const TaskCard: React.FC<TaskCardProps> = ({ taskName, progress }) => {
  const router = useRouter();

  // Handler for the detail button
  const handleDetailClick = () => {
    router.push(`/task-detail/${taskName}`); // Example route for task details
  };
  ...
  return (
    <div className="bg-red-300 p-4 rounded-lg shadow-md flex flex-col items-center w-64">
      ...
      <div className="flex items-center w-full mb-2">
        <FiCheckCircle className="text-white text-3xl mr-2" /> {/* Circular Check Icon */}
